              ...
              <Box sx={{ width: "100%" }}>
                  ...
                  (<Table responsive striped bordered hover size="sm">
                   ...
                   {allusers.map((user) => (
                          ...
                          <td className="action_btn">
                            <div>
                            <Button onClick={() => seeAllPets(user._id)}>See Pets</Button>
                            <Button onClick={() => updateThisUser(user._id)}>Update</Button>
                            <Button onClick={() => deleteThisUser(user._id)}>Delete</Button>
                          </div>
                          </td>
                        </tr>
                      ))}
                ...
                {allpets ? 
                  (<Table responsive striped bordered hover size="sm">
                   ...
                   {allpets.map((pet) => (
                          <tr>
                          <td><img className='table_img' src={pet.picture} alt="" /></td>
                          <td><p>{pet.name}</p></td>
                          <td><p>{pet.type}</p></td>
                          <td>{pet.adoptionStatus ? (<p>Adopted ❤️</p>) : (<p>Need a house 🙏</p>)}</td>
                          <td className="action_btn">
                            <div>
                            <Button onClick={() => updateThisPet(pet._id)}>Update</Button>
                            <Button onClick={() => deleteThisPet(pet._id)}>Delete</Button>
                          </div>
                          </td>
                        </tr>
                      ))}
